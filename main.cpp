...
bool ifcolidesup(SDL_Rect player, SDL_Rect* platform)  // sprawdzenie, czy nie ma kolizji od gÛry
{
	for (int i = 0; i < player.w; i++)
	{
...
int main(int argc, char** argv) {
	...
	char text[128];
	int gold = SDL_MapRGB(screen->format, 0xFF, 0xD7, 0x00);  // ustawienie kolorÛw 
	...
	factorY = FACTORY;
	speed = BASESPEED;
	posY = platformy[0].y-POS_ABOVE_1ST;    // ustawianie parametrÛw startowych
	jump = 0;
	djump = 1;
	djumpreq = 0;
	dash = 1;
	...
	while (!quit) {
		...
		while (SDL_PollEvent(&event)) {
			...
			case SDL_KEYUP:
				...
				else if (event.key.keysym.sym == SDLK_RIGHT)  // gracz przesta≥ poruszac sie w podstawowym sterowaniu
				{
					if (steer == 0)
					{
						keypressed = 0;
